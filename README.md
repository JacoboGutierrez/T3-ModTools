...
<span style="color:green">Note: Mods that modify the Modded Launcher will cause the game to crash the first time the modifying mod is compiled. After the initial crash, a new game instance opens automatically, and the game starts without issues. This will not happen again unless a new mod that modifies the launcher is installed.</span>
