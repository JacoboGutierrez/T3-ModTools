...
class BuildCleanupTests(unittest.TestCase):
    ...
    def test_top_mod_has_priority_for_duplicate_files(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            game_folder = root / "game"
            mods_folder = game_folder / "mods"
            ...
            for mod_id in mod_ids:
                project_folder = mods_folder / mod_id
                shared_file = project_folder / "files" / "sound44" / "shared.txt"
                shared_file.parent.mkdir(parents=True)
                shared_file.write_text(mod_id, encoding="utf-8")
                projects.append(ModProject(mod_id, mod_id, project_folder, enabled=True))

            result = manager.build(projects)

            with zipfile.ZipFile(result.output) as archive:
                self.assertEqual(
                    b"t3_enhanced_sfx",
                    archive.read("sound44/shared.txt"),
                )
            self.assertEqual("t3_enhanced_sfx", result.conflicts[-1].winning_mod)
            state = json.loads(manager.load_order_path.read_text(encoding="utf-8"))
            self.assertEqual(
                {
                    "format_version": 1,
                    "mods": [{"id": mod_id, "enabled": True} for mod_id in mod_ids],
                },
                state,
            )
